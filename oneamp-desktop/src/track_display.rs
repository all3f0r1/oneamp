...
impl TrackDisplay {
    ...
    pub fn get_artist(track: &TrackInfo) -> String {
        track.artist.clone().unwrap_or_else(|| "Unknown Artist".to_string())
    }
    
    /// Get display title (song name only) for a track
    pub fn get_title_only(track: &TrackInfo) -> String {
        track.title.clone().unwrap_or_else(|| Self::filename_fallback(&track.path))
    }
    
    /// Get display album for a track
    pub fn get_album(track: &TrackInfo) -> String {
        track.album.clone().unwrap_or_else(|| "Unknown Album".to_string())
    }
