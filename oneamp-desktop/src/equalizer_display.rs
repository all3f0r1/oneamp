        ...
        for (i, &gain) in eq_gains.iter().enumerate() {
            if i < self.peak_values.len() {
                ...
                if gain.abs() > self.peak_values[i] {
                    ...
                    // Decay peak
                    self.peak_values[i] =
                        (self.peak_values[i] - self.peak_decay_speed * dt).max(0.0);
                }
            }
        }
    }
            ...
            ui.horizontal(|ui| {
                ...
                for (i, gain) in eq_gains.iter_mut().enumerate() {
                    ui.vertical(|ui| {
                        ...
                        ui.label(egui::RichText::new(freq_label).size(10.0).color(
                            Theme::color32(&theme.colors.display_text).linear_multiply(0.7),
                        ));
...
fn render_eq_slider_3d(ui: &mut Ui, theme: &Theme, value: &mut f32, peak_value: f32) -> bool {
    let mut changed = false;
    ...
    if ui.is_rect_visible(rect) {
        ...
        let track_width = 8.0;
        let track_rect =
            Rect::from_center_size(rect.center(), Vec2::new(track_width, slider_height));
        ...
        if response.dragged() {
            if let Some(pointer_pos) = response.interact_pointer_pos() {
                let normalized =
                    1.0 - ((pointer_pos.y - rect.top()) / slider_height).clamp(0.0, 1.0);
        ...
        VisualEffects::button_3d(painter, thumb_rect, thumb_color, response.dragged(), 4.0);
