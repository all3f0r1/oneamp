    ...
    pub fn load_from_track(&mut self, track_path: &Path, ctx: &egui::Context) {
        ...
        match extract_album_art(track_path) {
            Some(image_data) => {
                ...
                self.texture =
                    Some(ctx.load_texture("album_art", image_data, TextureOptions::LINEAR));
            }
            None => {
    ...
    pub fn render(&self, ui: &mut Ui, theme: &Theme, size: f32) {
        if let Some(texture) = &self.texture {
            ...
            if ui.is_rect_visible(rect) {
                ...
                // Main album art
                let art_rect = Rect::from_min_size(rect.min, Vec2::splat(size));
...
fn draw_reflection(painter: &Painter, texture: &TextureHandle, rect: Rect) {
    ...
    for i in 0..steps {
        ...
        painter.image(
            texture.id(),
            step_rect,
            egui::Rect::from_min_max(egui::pos2(0.0, uv_top), egui::pos2(1.0, uv_bottom)),
