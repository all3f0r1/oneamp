...
pub fn button_3d(ui: &mut Ui, theme: &Theme, text: &str, icon: Option<&str>) -> Response {
    ...
    if ui.is_rect_visible(rect) {
        ...
        // 3D button effect
        VisualEffects::button_3d(painter, rect, base_color, response.clicked(), 4.0);
...
pub fn progress_bar_fancy(ui: &mut Ui, theme: &Theme, progress: f32, time: f32) -> Response {
        ...
        if progress > 0.0 {
            ...
            if shine_x > fill_rect.left() && shine_x < fill_rect.right() {
                let shine_rect = Rect::from_min_max(
                    Pos2::new(
                        (shine_x - shine_width / 2.0).max(fill_rect.left()),
                        fill_rect.top(),
                    ),
                    Pos2::new(
                        (shine_x + shine_width / 2.0).min(fill_rect.right()),
                        fill_rect.bottom(),
                    ),
                );
...
pub fn slider_3d(
    ...
    if ui.is_rect_visible(rect) {
        ...
        // Track
        let track_rect = Rect::from_center_size(rect.center(), Vec2::new(rect.width(), 6.0));

        painter.rect_filled(track_rect, 3.0, Theme::color32(&theme.colors.progress_bg));
...
pub fn lcd_display(ui: &mut Ui, theme: &Theme, text: &str, large: bool) -> Response {
    let font_size = if large {
        theme.fonts.timer_size
    } else {
        theme.fonts.track_info_size
    };
    let desired_size = Vec2::new(ui.available_width().min(300.0), font_size + 16.0);
...
pub fn metallic_panel(ui: &mut Ui, theme: &Theme, content: impl FnOnce(&mut Ui)) {
    ...
    if ui.is_rect_visible(rect) {
        ...
        VisualEffects::metallic_panel(painter, rect, Theme::color32(&theme.colors.panel_bg), 6.0);
    }
