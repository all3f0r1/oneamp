...
- **Audio engine** — Symphonia 0.6 decoding (MP3, FLAC, OGG / Vorbis, WAV, AAC, M4A / MP4, ALAC, AIFF, CAF, MKA) straight into cpal — no rodio mixer. The stream opens at the track's **native sample rate** in the device's most precise format (F32 > I32 > I24 > I16); only if the device refuses that rate does a band-limited FFT resampler (rubato) step in. With EQ flat, no preamp / ReplayGain, balance centred and volume at 100 %, every DSP stage is skipped and OneAmp hands the device **bit-perfect** samples for integer sources up to 24 bits (TPDF dither only when something actually requantizes). That guarantee stops at the OS: cpal opens the device in shared mode (WASAPI shared on Windows, the ALSA default / PipeWire / PulseAudio on Linux, CoreAudio on macOS), so the system mixer may still resample or process the signal before the DAC — set the system output format to match your files for a true end-to-end path. 32-bit integer PCM is rounded by the f32 pipeline. Sources with more channels than the device are cut down to the first channels (no downmix matrix: a 5.1 file on a stereo device loses centre, LFE and surrounds). Gapless: encoder delay / padding trimmed (MP3/AAC), preloaded decoder swapped into the live stream. Volume ≤ 100 % applies in the output callback (instant, 5 ms smoothing); boost above 100 % goes ahead of a 2 ms-lookahead 0 dBFS limiter that is a pure delay until something passes full scale — gain changes are linear ramps, never steps. Lock-free SPSC ring to the real-time callback, which runs with real-time priority on Linux (rtkit).
...
- **Stereo balance** — Winamp-style: unity at centre (a bit-exact no-op), panning attenuates only the opposite channel linearly to silence.
...
- The Rust audio crowd — Symphonia, cpal, rubato, rodio, fundsp, egui — without whom this would still be a daydream on a sticky note.
